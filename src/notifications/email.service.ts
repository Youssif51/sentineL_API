...
export class EmailService {
  ...
  async sendPriceAlert(data: PriceAlertData): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to send price alert to ${data.to}`, (err as Error).stack);
      throw err; // بنعمل throw هنا عشان الـ BullMQ يفهم إن الـ Job فشلت فيعيد المحاولة
    }
  }

  // ميثود الـ Lockout جاهزة للاستخدام بنفس المنطق
  async sendLockoutNotification(email: string, unlocksAt: Date): Promise<void> {
    try {
      await this.transporter.sendMail({
        ...
        html: `<h2>Account Temporarily Locked</h2><p>Unlocks at: ${unlocksAt.toLocaleString()}</p>`,
      });
    } catch (err) {
      this.logger.error(`Failed to send lockout email`, (err as Error).message);
      throw err;
    }
  }
  ...
  private buildAlertHtml(d: PriceAlertData): string {
    return `<!DOCTYPE html><html><body style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:20px">
      ...
      <a href="${d.productUrl}" style="background:#3182ce;color:white;padding:12px 24px;text-decoration:none;border-radius:4px;display:inline-block">View on ${d.store}</a>
      
      <p style="color:#999;font-size:0.8em;margin-top:24px">
        SeerPrice — Egyptian Electronics Price Tracker<br>
        <a href="https://seerprice.com/unsubscribe?email=${encodeURIComponent(d.to)}" style="color:#3182ce">Unsubscribe from these alerts</a>
      </p>
      </body></html>`;
  }
}
